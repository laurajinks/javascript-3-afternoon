...
//Code Here
let totalPopulation = populations.reduce(function(total, index, arr){
  return total += index;
})
...
let bobsTotal = purchases.filter(obj => obj.owner === 'Bob').map(obj => obj.price).reduce(function(total, item){
  return total += item;
})
